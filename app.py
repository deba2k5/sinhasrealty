...
_client = None
_db = None
SYNC_META_COLLECTION = '_sync_metadata'
...
def mark_collection_updated(collection_name):
    """Persist collection update timestamps in MongoDB for cross-device sync checks."""
    try:
        get_db()[SYNC_META_COLLECTION].update_one(
    ...
    try:
        ...
        if filename.endswith('.csv'):
            ...
            if records:
                ...
                mark_collection_updated(collection_name)
...
def handle_upload_occupancy():
    ...
    try:
        ...
        if records:
            ...
            mark_collection_updated('property details data')
    ...
    try:
        ...
        if "VERWALTUNG CONTACTS" in xls.sheet_names:
            ...
            if records:
                ...
                mark_collection_updated('verwaltung_contacts')
        ...
        if "AGENCY SUMMARY" in xls.sheet_names:
            ...
            if agency_records:
                ...
                mark_collection_updated('agency_summary')
        ...
        for sheet_name, col_name, header_idx, include_skeleton in sheets_info:
            if sheet_name in xls.sheet_names:
                ...
                if records:
                    ...
                    mark_collection_updated(col_name)
...
@app.route('/api/collection-last-updated', methods=['GET'])
def collection_last_updated():
    collection = request.args.get('collection', 'property details data').strip()
    if not collection:
        return jsonify({'success': False, 'message': 'collection is required'}), 400

    try:
        doc = get_db()[SYNC_META_COLLECTION].find_one({'collection': collection})
        updated_at = doc.get('updated_at') if doc else None
        return jsonify({
            'success': True,
            'collection': collection,
            'lastUpdated': datetime_to_iso_z(updated_at)
        })
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
        ...
        if col_order_doc:
            ...
            if collection_name == 'reservation_details':
                column_order = [
                    ...
                    'Notice Period', 'Minimum Stay', 'Remarks'
                ]
            elif docs:
